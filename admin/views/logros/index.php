...
.logros-title small{ color:#6c757d; }
...
.logros-card .card-header h6{ color:#fff; }

.form-control{ border-radius:10px; }

/* Tabla */
.logros-table-wrap{ max-height: 70vh; overflow:auto; }
.logros-table{
  border-collapse: collapse;
  width:100%;
}
.logros-table th, .logros-table td{
...
.logros-table tbody tr:hover{ background: rgba(0,0,0,.03); }
...
.btn-pill i{ margin-right:4px; font-size:.75rem; }
...
<input type="hidden" name="csrf" value="<?= csrf_token() ?>">
            ...
            <div class="form-group">
              ...
              <input type="number" name="meta_asistencias" id="meta_asistencias" class="form-control" required min="1">
            </div>
        ...
        <div class="card-body">
          <div class="table-responsive logros-table-wrap">
            <table class="table table-sm mb-0 logros-table">
              ...
              <tbody>
                <?php if (empty($logros)): ?>
                  <tr><td colspan="7" class="text-center text-muted py-4">No hay logros aún.</td></tr>
                <?php else: ?>
                  <?php foreach ($logros as $l): ?>
                    <?php
                      $iconFa = trim((string)$l['icono_fa']);
                      if ($iconFa === '') $iconFa = 'fa-medal';
                    ?>
                    <tr>
