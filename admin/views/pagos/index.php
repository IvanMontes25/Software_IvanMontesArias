...
function pager(int $page, int $total): string{
  ...
  if ($start > 1) {
    ...
    if ($start > 2) $h .= '<li class="page-item disabled"><span class="page-link">…</span></li>';
  }
  ...
  if ($end < $total) {
    if ($end < $total - 1) $h .= '<li class="page-item disabled"><span class="page-link">…</span></li>';
...
<style>
.pay-card{border:0;border-radius:1rem;overflow:hidden}
.pay-card .card-header{background:linear-gradient(90deg,#4e73df,#1cc88a);color:#fff}
.pay-table-wrap{max-height:70vh;overflow:auto}
.pay-table thead th{font-size:1rem;font-weight:600}
.pay-table tbody td{font-size:1.03rem;padding:.75rem .5rem}
...
.pay-table td.text-right{text-align:right!important}
.actions .btn{font-size:.78rem;padding:.25rem .5rem;border-radius:999px}
.actions.historial .btn{font-size:.75rem;padding:.22rem .45rem}
</style>
  ...
  <form method="get" class="m-0 mt-2 mt-md-0">
    ...
    <div class="attendance-search">
      ...
      <input
        ...
        name="q"
        value="<?= e($q); ?>"
        placeholder="Buscar por nombre, CI o membresía…"
        autocomplete="off"
      >
...
<?php else: $n=$offset+1; foreach($members as $m): ?>
<tr>
  <td><?= $n++; ?></td>
  <td><?= e($m['fullname']); ?></td>
  <td><?= e($m['ci']); ?></td>
...
?>
  ...
  <td class="actions historial">
    <a class="btn btn-outline-primary" href="historial_cliente.php?user_id=<?= (int)$m['user_id']; ?>">Ver historial</a>
...
.page-header-inner{
  max-width: 1200px; /* 🔥 MISMO ANCHO QUE TABLAS */
  margin: 0 auto;
  text-align: center;
  color: #fff;
}
