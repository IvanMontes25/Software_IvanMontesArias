...
if (isset($_GET['user_id']) && ctype_digit($_GET['user_id'])) {
    ...
    die("<div class='alert alert-danger text-center mt-5'>ID de cliente no valido.</div>");
}
...
$stmtC = $db->prepare("SELECT fullname FROM members WHERE user_id = ? LIMIT 1");
if ($stmtC) {
    $stmtC->bind_param("i", $user_id);
    $stmtC->execute();
    $stmtC->bind_result($fnameResult);
    if ($stmtC->fetch()) {
        $clienteNombre = $fnameResult;
    }
    $stmtC->close();
}
...
/* =====================================================
   ASISTENCIAS DEL MES (con hora)
===================================================== */
$sql = "SELECT curr_date, curr_time
        FROM attendance
        WHERE user_id = ?
          AND YEAR(curr_date) = ?
          AND MONTH(curr_date) = ?
...
while ($row = $res->fetch_assoc()) {
    $presentDays[$row['curr_date']] = $row['curr_time'];
    $list[] = $row;
}
$stmt->close();

$totalMonth = count($presentDays);

/* =====================================================
   TOTAL HISTORICO
===================================================== */
$totalStmt = $db->prepare("SELECT COUNT(*) AS total FROM attendance WHERE user_id = ?");
$totalStmt->bind_param("i", $user_id);
$totalStmt->execute();
$totalAsistencias = (int) $totalStmt->get_result()->fetch_assoc()['total'];
$totalStmt->close();
...
$firstDow = (int) date('N', strtotime("$year-$month-01"));
...
$pageTitle = 'Asistencias - ' . h($clienteNombre);
