...
// Datos del cliente
$stmt = $db->prepare("SELECT fullname, username FROM members WHERE user_id = ?");
...
// ===== 3) Total de asistencias del cliente =====
$total = 0;
if ($stmt = $db->prepare("SELECT COUNT(*) AS cnt FROM attendance WHERE user_id = ?")) {
  $stmt->bind_param("i", $member_id);
  $stmt->execute();
...
// ===== 4) Definición de logros desde BD =====
$achievements = [];
if (
  $stmt = $db->prepare("
...
// ===== 5) Calcular siguiente logro y progreso =====
$next = null;
...
if ($next) {
  ...
  $progress = max(0, min(100, (int) round((($total - $prevGoal) / $range) * 100)));
}
