...
if (!$db instanceof mysqli) {
  ...
  exit('No se encontró conexión a la base de datos.');
}

/* =========================
   ELIMINAR EQUIPO (POST + CSRF)
========================= */
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_equipo'])) {
  ...
  $id = filter_input(INPUT_POST, 'delete_equipo', FILTER_VALIDATE_INT);
  ...
  $check = $db->prepare("SELECT id FROM equipment WHERE id = ? LIMIT 1");
  ...
  if ($res->num_rows === 0) {
    echo json_encode(['ok' => false, 'msg' => 'No se encontró el equipo']);
    exit;
  }
...
/* =========================
   LISTAR EQUIPOS
========================= */
$stmt = $db->prepare("SELECT * FROM equipment ORDER BY id ASC");
$stmt->execute();
