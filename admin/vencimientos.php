...
$pageTitle = 'Gestion de Vencimientos';

function e($s){
    return htmlspecialchars((string)($s ?? ''), ENT_QUOTES, 'UTF-8');
}

$diasFilter = isset($_GET['dias']) ? (int)$_GET['dias'] : 7;
$opcionesDias = [7, 15, 30, 60];
if (!in_array($diasFilter, $opcionesDias)) {
    $diasFilter = 7;
}

$res = $db->query("SELECT user_id, fullname, username, ci, contact FROM members ORDER BY fullname ASC");
...
foreach ($clientes as $c) {
    $m = membership_last($db, (int)$c['user_id']);
    if (!$m) continue;
    $diasRestantes = membership_days_left($m);
    if ($diasRestantes < 0) {
        $vencidos[] = ['cliente' => $c, 'm' => $m, 'dias' => $diasRestantes];
    } elseif ($diasRestantes >= 0 && $diasRestantes <= $diasFilter) {
        $porVencer[] = ['cliente' => $c, 'm' => $m, 'dias' => $diasRestantes];
    }
}

usort($porVencer, function($a, $b){ return $a['dias'] <=> $b['dias']; });
usort($vencidos, function($a, $b){ return strcmp(membership_end_date($b['m']), membership_end_date($a['m'])); });
