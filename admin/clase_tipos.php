<?php
/* admin/clase_tipos.php — CRUD de Tipos de Clase (solo admin) */
require_once __DIR__ . '/../core/bootstrap.php';
require_once __DIR__ . '/../core/auth.php';
require_once __DIR__ . '/../core/roles.php';
require_modulo('administracion');

// PROCESAR POST *ANTES* de incluir header/sidebar
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'crear') {
        ...
        if ($nombre === '') {
            ...
            $st = $db->prepare("INSERT INTO clase_tipos (nombre, color, descripcion) VALUES (?, ?, ?)");
            $st->bind_param("sss", $nombre, $color, $desc);
            if ($st->execute()) {
                $_SESSION['flash_ok'] = "Tipo \"" . htmlspecialchars($nombre) . "\" creado exitosamente.";
                require_once __DIR__ . '/../core/audit.php';
                if (function_exists('registrar_auditoria')) {
                    registrar_auditoria($db, 'crear_tipo_clase', "Creó tipo de clase: $nombre", 'clases');
                }
            } else {
                $_SESSION['flash_error'] = 'Error al crear: ' . $db->error;
            }
            $st->close();
        }
    }
    ...
    if ($action === 'editar') {
        ...
        if ($id <= 0 || $nombre === '') {
            ...
            $st = $db->prepare("UPDATE clase_tipos SET nombre = ?, color = ?, descripcion = ?, activo = ? WHERE id = ?");
            $st->bind_param("sssii", $nombre, $color, $desc, $activo, $id);
            if ($st->execute()) {
                $_SESSION['flash_ok'] = "Tipo \"" . htmlspecialchars($nombre) . "\" actualizado.";
            } else {
    ...
    if ($action === 'eliminar') {
        $id = (int) ($_POST['id'] ?? 0);
        if ($id > 0) {
            $chk = $db->prepare("SELECT COUNT(*) AS total FROM clases_sesiones WHERE tipo_clase_id = ?");
            $chk->bind_param("i", $id);
            $chk->execute();
            ...
            if ($count > 0) {
                $_SESSION['flash_error'] = "No se puede eliminar: hay $count clase(s) usando este tipo. Desactívalo en su lugar.";
            } else {
                $del = $db->prepare("DELETE FROM clase_tipos WHERE id = ?");
                $del->bind_param("i", $id);
                if ($del->execute()) {
                    $_SESSION['flash_ok'] = 'Tipo de clase eliminado correctamente.';
                } else {
    ...
    header('Location: clase_tipos.php');
    exit;
}


// SOLO GET: obtener datos y luego renderizar HTML

$tipos = [];
$res = $db->query("SELECT * FROM clase_tipos ORDER BY activo DESC, nombre ASC");
...
// Paleta de colores para los modales
$colores = ['#ef4444', '#f59e0b', '#10b981', '#3b82f6', '#8b5cf6', '#ec4899', '#06b6d4', '#6366f1', '#84cc16', '#f97316'];

$pageTitle = 'Tipos de Clase';
$page = 'clase-tipos';
